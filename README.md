...
**[Live demo](https://eu-reg-verifier-863444617243.us-central1.run.app)** *(Cloud Run hosting, password protected)*

![Screenshot of the app answering a GDPR compliance question with article citations](assets/screenshot.png)

## Why it's useful

- **Answers you can verify.** Each answer cites the exact article and shows the source text.
...
- `ingest.py` splits the GDPR into one chunk per article, embeds each with
  `mistral-embed`, and builds a FAISS index (committed, so the app loads
  instantly and never rebuilds at runtime).
- `rag.py` embeds your question, retrieves the most relevant articles, checks
...
The regulation text is EU law, so an EU model provider is a natural fit. Mistral also
publishes open-weight models, and nothing in `rag.py` is tied to the hosted
API, so the same pipeline can run against a self-hosted model when data
cannot leave the buyer's infrastructure. The judge and answer calls are
small, well-defined tasks that a compact model handles well, which keeps the
cost per question low. More in [BUSINESS.md](BUSINESS.md).
...
Contains the text of Regulation (EU) 2016/679 (GDPR), consolidated version,
from EUR-Lex © European Union, reused under Commission Decision 2011/833/EU /
CC BY 4.0. Text has been segmented (chunked) for retrieval; meaning unaltered.

Only the Official Journal of the European Union (electronic edition, on
EUR-Lex) is authentic and produces legal effects. This tool answers what the
regulation text says, with citations. It is not legal advice.
...
An online business rarely faces just one regulation. The engine is
regulation-agnostic: the same grounded, cited, no-bluffing approach extends
to ePrivacy, the DSA, NIS2 and the AI Act by adding text and config, not by
rewriting.

## More

- [ARCHITECTURE.md](ARCHITECTURE.md): the full pipeline, why the judge step
  exists, and how the regulation-agnostic design works.
- [EVALUATION.md](EVALUATION.md): real results from a 9-question test set,
  including the one known limitation it surfaced.
- [BUSINESS.md](BUSINESS.md): who this is for and why this stack.
